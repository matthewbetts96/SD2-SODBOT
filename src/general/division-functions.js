...
module.exports.rdiv = rdiv = (message, input, isRandom) => {
  ...
  let divs = "";
  side === "axis" ? (divs = common.axis) : (divs = common.allies);
...
module.exports.banning_unbanningDivs = (message, msgInput, unbanBool) => {
  ...
  msgInput.forEach((i, index) => {
    ...
    if (!mergedSides.hasOwnProperty(msgInput[index])) {
      ...
      if (msgInput[index] === `${config.prefix}bandiv`) {
        ...
        common.say(
          message,
          `I don't know what that division is, did you mean ***${common.lexicalGuesser(
            msgInput[index],
            mergedSides
          )}*** instead of ***${
            msgInput[index]
  ...
  unbanBool
    ? common.say(
        message,
        "``The following divisions have been unbanned: " + divsBanned + "``"
      )
    : common.say(
        message,
        "``The following divisions have been banned: " + divsBanned + "``"
      );
  bannedDivs(message);
};
