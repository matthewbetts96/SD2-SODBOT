...
module.exports.info = message => {
  ...
  table.addRow(
    "Find any bugs or want to contribute? Please make an issue/PR on the Github page below."
  );
...
function specialPiat(message) {
  ...
  const k = Math.random();
  k < 0.9
    ? (common.reply(message, `You hit!`, true),
      setTimeout(() => {
        common.reply(message, `Just kidding, you didn't.`, true);
      }, 5000))
    : (common.say(
        message,
